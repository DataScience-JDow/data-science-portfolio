...
def validate_model(data: pd.DataFrame) -> dict:
        ...
        fold_rows.append(
            {
                ...
                "blended_typewell_gr_rmse": rmse(target, blended_prediction),
            }
        )
    ...
    return {
        "method": "typewell_gr_residual_hgb",
        ...
        "baseline_last_known_tvt_rmse": rmse(target, baseline),
        "typewell_gr_residual_rmse": rmse(target, final_prediction),
        "previous_residual_model_rmse": 15.33328070147783,
        "absolute_rmse_improvement_vs_previous": 15.33328070147783
        - rmse(target, final_prediction),
        "relative_rmse_improvement_pct_vs_previous": (
            (15.33328070147783 - rmse(target, final_prediction))
            / 15.33328070147783
            * 100.0
        ),
...
def main() -> None:
    ...
    metrics = validate_model(training_features)
    model = train_final_model(training_features)
    submission = build_submission(model)

    metrics_path = REPORTS_DIR / "typewell-gr-residual-metrics.json"
    submission_path = SUBMISSIONS_DIR / "typewell_gr_residual_submission.csv"
    ...
    print(
        "Previous residual RMSE: "
        f"{metrics['previous_residual_model_rmse']:.5f}"
    )
    print(
        "Typewell GR residual RMSE: "
        f"{metrics['typewell_gr_residual_rmse']:.5f}"
    )
    print(
        "Relative improvement vs previous: "
        f"{metrics['relative_rmse_improvement_pct_vs_previous']:.2f}%"
    )
