...
def main() -> None:
    ...
    train_features = train_df.drop(columns=[target_column, "id"]).copy()
    test_features = test_df.drop(columns=["id"]).copy()

    X = add_missing_indicators(train_features)
    y = train_df[target_column].copy()
    X_test = add_missing_indicators(test_features)
    ...
    output_path = SUBMISSIONS_DIR / "histgb_balanced_missing_flags_submission.csv"
